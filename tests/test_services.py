...
def test_update_metrics_config_disabled(settings):
    ...
    with (
        patch("wb.cloud_agent.services.metrics.try_stop_and_disable_service") as mock_stop,
...
def test_stop_metrics_health_monitor_signals_and_forgets_monitor(settings):
    ...
    with (
        ...
        assert settings.provider_name not in metrics._monitor_threads
        assert settings.provider_name not in metrics._monitor_stop_events
