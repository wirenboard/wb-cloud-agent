...
from wb.cloud_agent.services.tunnel import reload_frpc, update_tunnel_config
...
CLOUD_CONFIG = (
    "# Autogenerated config!\n\n[common]\nserver_addr = 1.2.3.4\nmeta_tunnel_token = tok\nuser = SN\n\n"
    "[ssh]\ntype = tcp\n"
)


def admin_api(port: int) -> str:
    return f"admin_addr = 127.0.0.1\nadmin_port = {port}\nadmin_user = wb-cloud-agent\nadmin_pwd = tok\n"


def with_admin_api(port: int) -> str:
    return CLOUD_CONFIG.replace("[common]\n", "[common]\n" + admin_api(port))


def assert_frpc_restarted(settings, tunnel_mocks):
    tunnel_mocks["stop"].assert_called_once_with(settings.frp_service)
    tunnel_mocks["reset_failed"].assert_called_once_with(settings.frp_service)
    tunnel_mocks["start"].assert_called_once_with(settings.frp_service)


@pytest.mark.parametrize("payload", [{"config": CLOUD_CONFIG}, {"config": CLOUD_CONFIG, "restart": True}])
def test_update_tunnel_config_restarts_frpc_with_admin_api(isolated_provider_runtime, tunnel_mocks, payload):
    """Без флага (старое облако) и с restart=True — рестарт без попытки reload; admin API включает агент."""
    ...
    assert settings.frp_config.stat().st_mode & 0o777 == 0o600
    tunnel_mocks["reload"].assert_not_called()
    assert_frpc_restarted(settings, tunnel_mocks)
    tunnel_mocks["write_link"].assert_called_once_with(settings, UNKNOWN_LINK, mock_mqtt)


def test_update_tunnel_config_hot_reloads_on_running_port(isolated_provider_runtime, tunnel_mocks):
    settings = isolated_provider_runtime
    settings.frp_config.write_text("[common]\n" + admin_api(7110) + "server_addr = 0.0.0.0\n")

    update_tunnel_config(settings, {"config": CLOUD_CONFIG, "restart": False}, MagicMock())

    assert settings.frp_config.read_text() == with_admin_api(7110)
    tunnel_mocks["reload"].assert_called_once_with(settings.frp_config)
    tunnel_mocks["stop"].assert_not_called()
    tunnel_mocks["start"].assert_not_called()


def test_update_tunnel_config_restarts_on_same_port_when_reload_fails(
    isolated_provider_runtime, tunnel_mocks
):
    settings = isolated_provider_runtime
    settings.frp_config.write_text("[common]\n" + admin_api(7110))
    tunnel_mocks["reload"].return_value = False

    update_tunnel_config(settings, {"config": CLOUD_CONFIG, "restart": False}, MagicMock())

    assert settings.frp_config.read_text() == with_admin_api(7110)
    tunnel_mocks["port_taken"].assert_called_once_with("127.0.0.1", 7110)
    assert_frpc_restarted(settings, tunnel_mocks)


@pytest.mark.parametrize("previous", [None, "[common]\nserver_addr = 0.0.0.0\n"])
def test_update_tunnel_config_restarts_frpc_without_admin_api_yet(
    isolated_provider_runtime, tunnel_mocks, previous
):
    """Первое событие после обновления агента: у работающего frpc нет admin API, reload невозможен."""
    settings = isolated_provider_runtime
    if previous is not None:
        settings.frp_config.write_text(previous)
...
def test_update_tunnel_config_takes_next_free_port_after_stopping_frpc(
    ...
    """Проба после остановки своего frpc (иначе он сам считался бы конфликтом): прежний порт первым."""
    settings = isolated_provider_runtime
    settings.frp_config.write_text("[common]\n" + admin_api(7110))
    tunnel_mocks["port_taken"].side_effect = lambda addr, port: port in (7110, 7106)
    order = MagicMock()
    order.attach_mock(tunnel_mocks["stop"], "stop")
    order.attach_mock(tunnel_mocks["reset_failed"], "reset_failed")
    order.attach_mock(tunnel_mocks["port_taken"], "port_taken")
    order.attach_mock(tunnel_mocks["start"], "start")

    update_tunnel_config(settings, {"config": CLOUD_CONFIG, "restart": True}, MagicMock())

    assert order.mock_calls == [
        call.stop(settings.frp_service),
        call.reset_failed(settings.frp_service),
        call.port_taken("127.0.0.1", 7110),
        call.port_taken("127.0.0.1", 7106),
        call.port_taken("127.0.0.1", 7107),
        call.start(settings.frp_service),
    ]
    assert settings.frp_config.read_text() == with_admin_api(7107)


def test_update_tunnel_config_starts_without_admin_api_when_no_port_is_free(
    isolated_provider_runtime, tunnel_mocks, caplog
):
    """Все порты заняты — frpc без admin API лучше, чем frpc в рестарт-цикле systemd."""
    settings = isolated_provider_runtime
    tunnel_mocks["port_taken"].return_value = True

    with caplog.at_level(logging.WARNING):
        update_tunnel_config(settings, {"config": CLOUD_CONFIG}, MagicMock())

    assert settings.frp_config.read_text() == CLOUD_CONFIG
    assert tunnel_mocks["port_taken"].call_count == 10
    assert "frpc runs without admin API" in caplog.text
    assert_frpc_restarted(settings, tunnel_mocks)


@pytest.mark.usefixtures("tunnel_mocks")
def test_update_tunnel_config_needs_tunnel_token_for_admin_api(isolated_provider_runtime):
    settings = isolated_provider_runtime
    config = CLOUD_CONFIG.replace("meta_tunnel_token = tok\n", "")

    update_tunnel_config(settings, {"config": config}, MagicMock())

    assert settings.frp_config.read_text() == config


@pytest.mark.parametrize("returncode, expected", [(0, True), (1, False)])
def test_reload_frpc_exit_code(mock_subprocess_run, tmp_path, caplog, returncode, expected):
    mock_subprocess_run.return_value = MagicMock(returncode=returncode, stdout="", stderr="boom")

    with caplog.at_level(logging.WARNING):
        assert reload_frpc(tmp_path / "frpc.conf") is expected

    assert mock_subprocess_run.call_args.args[0] == ["frpc", "reload", "-c", str(tmp_path / "frpc.conf")]
    assert ("frpc reload failed (exit 1): boom" in caplog.text) is not expected


@pytest.mark.parametrize("error", [TimeoutExpired(["frpc"], 60), FileNotFoundError("frpc")])
def test_reload_frpc_swallows_process_errors(mock_subprocess_run, tmp_path, error):
    mock_subprocess_run.side_effect = error

    assert reload_frpc(tmp_path / "frpc.conf") is False
