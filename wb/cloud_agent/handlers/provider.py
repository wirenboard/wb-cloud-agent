...
from wb.cloud_agent.utils import try_stop_and_disable_service
...
def unbind_provider(settings: AppSettings, _: dict, mqtt: MQTTCloudAgent) -> None:
    ...
    try_stop_and_disable_service(settings.frp_service)
    try_stop_and_disable_service(settings.metrics_service)
