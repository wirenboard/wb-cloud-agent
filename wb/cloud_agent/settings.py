...
class AppSettings:  # pylint: disable=too-many-instance-attributes disable=too-few-public-methods
    ...
    def __init__(
        ...
        missing, require_conf_file=True raises FileNotFoundError instead of falling back to the
        defaults; skip_conf_file=True leaves the file unread either way.
        """
        ...
        if not self.skip_conf_file:
            if self.config_file.exists():
                ...
                raise FileNotFoundError(f"Provider config not found: {self.config_file}")
